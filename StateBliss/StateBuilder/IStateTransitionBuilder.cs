...
//       
//        IStateTransitionBuilder<TState> Changing<TTriggerCommand, T>(T target, Expression<Func<T, OnStateEventGuardHandler<TState, TTriggerCommand>>> handler)
//            where T : class
//            where TTriggerCommand : TriggerCommand<TState>;
//        
...
//            where T : class;
//        
        IStateTransitionBuilder<TState> Changing<T, TTriggerContext>(T target, Expression<Func<T, StateChangeHandler<TState, TTriggerContext>>> handler)
            where T : class;
        
        IStateTransitionBuilder<TState> Changing<T>(T target, Expression<Func<T, StateChangeHandler<TState>>> handler)
            where T : class;
//        
//        IStateTransitionBuilder<TState> Changing<TTriggerCommand, T, TContext>(T target, Func<TContext> contextProvider, 
//            params Expression<Func<T, OnStateEventGuardHandler<TState, TTriggerCommand, TContext>>>[] handlers)
//            where T : class
//            where TTriggerCommand : TriggerCommand<TState>, new()
//            where TContext : GuardStateContext<TState>, new();
//        
//        IStateTransitionBuilder<TState> Changing<TTriggerCommand>(TState state, 
