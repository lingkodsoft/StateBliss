...
namespace StateBliss
{
    public interface IStateFromBuilder<TState> where TState : Enum
    {
        ...
        void OnEntering<T>(TState state, T target, Expression<Func<T, StateChangeHandler<TState>>> handler)
            where T : class;
        void OnEntered<T>(TState state, T target, Expression<Func<T, StateChangeHandler<TState>>> handler)
            where T : class;
        void OnExiting<T>(TState state, T target, Expression<Func<T, StateChangeHandler<TState>>> handler)
            where T : class;
        void OnExited<T>(TState state, T target, Expression<Func<T, StateChangeHandler<TState>>> handler)
            where T : class;
        void OnEditing<T>(TState state, T target, Expression<Func<T, StateChangeHandler<TState>>> handler)
            where T : class;
