...
namespace StateBliss
{
    ...
    public delegate void OnStateEventGuardHandler<in TContext>(TContext context)
        where TContext : GuardStateContext, new();
    
    public delegate void OnStateEventGuardHandler<TState, in TTriggerCommand>(TTriggerCommand command, GuardStateContext<TState> context)
        where TState : Enum
        where TTriggerCommand : TriggerCommand<TState>;

    public delegate void OnStateEventGuardHandler<TState, in TTriggerCommand, in TContext>(TTriggerCommand command, TContext context)
        where TState : Enum
